...
static void compose_impl(struct _xrender_data *xd, struct xrender_image *xrimg,
			 ...
			 struct xrender_image *mask, coord_t mask_dst,
	...
	// Remember: the mask has a 1-pixel border
	const auto mask_dst_x = to_i16_checked(dst_x1 - mask_dst.x + 1);
	const auto mask_dst_y = to_i16_checked(dst_y1 - mask_dst.y + 1);
...
#undef DOUBLE_TO_XFIXED
	...
	if (((img->color_inverted || img->dim != 0) && has_alpha) || img->corner_radius != 0) {
		...
		xcb_render_composite(xd->base.c, XCB_RENDER_PICT_OP_OVER, tmp_pict,
		       ...
		       to_i16_checked(dst_x1), to_i16_checked(dst_y1), tmpew, tmpeh);
		...
		xcb_render_composite(xd->base.c, op, inner->pict, mask_pict, result, 0,
		                     0, mask_dst_x, mask_dst_y, to_i16_checked(dst_x1),
...
static void compose(backend_t *base, void *img_data,
	...
	return compose_impl(xd, img_data, dst_x1, dst_y1, dst_x2, dst_y2, mask, mask_dst, reg_paint, reg_visible, xd->back[2]);
}
